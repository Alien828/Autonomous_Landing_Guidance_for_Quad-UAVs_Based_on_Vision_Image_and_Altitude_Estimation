...
class YOLOLoss(nn.Module):
    ...
    def box_nwd(self, pred, target, eps=1e-7, constant=12.8):
        ...
        center1 = pred[..., :2]  # 预测框中心坐标
        center2 = target[..., :2]  # 目标框中心坐标

        whs = center1[..., :2] - center2[..., :2] # 预测框中心坐标-目标框中心坐标

        center_distance = whs[..., 0] * whs[..., 0] + whs[..., 1] * whs[..., 1] + eps  #x差与y差平方和+eps

        w1 = pred[..., 2] + eps    # 预测框宽+eps
        h1 = pred[..., 3] + eps    # 预测框高+eps
        w2 = target[..., 2] + eps  # 目标框宽+eps
        h2 = target[..., 3] + eps  # 目标框高+eps

        wh_distance = ((w1 - w2) ** 2 + (h1 - h2) ** 2) / 4

        wasserstein_2 = center_distance + wh_distance  # 2阶wasserstein距离
    ...
    def forward(self, l, input, targets=None):
        ...
        if n != 0:
            ...
            #---------------------------------------------------------------#
            # NWD
            nwd = self.box_nwd(pred_boxes, y_true[..., :4]).squeeze()
            loss_loc    = torch.mean((1 - nwd)[obj_mask] * box_loss_scale[obj_mask])
            # iou
            iou = self.box_iou(pred_boxes, y_true[..., :4]).squeeze()
            # ciou
